...
function generatePayments(rng: () => number, orders: Record<string, unknown>[]) {
  return orders
    ...
    .map((o, i) => ({
      ...
      payment_link: `https://pay.wasl.app/${o.id}`,
      ...
      confirmed_by: o.payment_status === "confirmed" ? "owner@wasl.app" : null,
...
function generateAiOutputs(rng: () => number, conversations: Record<string, unknown>[]) {
  return conversations.slice(0, 18).map((c, i) => ({
    ...
    approved: i % 3 !== 0,
    approved_by: i % 3 !== 0 ? "owner@wasl.app" : null,
...
function generateSettings(): Record<string, unknown>[] {
  return [
    ...
    { key: "owner_email", value: "owner@wasl.app", updated_at: isoAt(0) },
    { key: "support_phone", value: "+971 50 000 0000", updated_at: isoAt(0) },
    { key: "company_legal_name", value: "Wasl Commerce FZ-LLC", updated_at: isoAt(0) },
