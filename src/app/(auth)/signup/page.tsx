...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // In demo mode, go straight to dashboard; otherwise verify email
      if (data.demo) {
    ...
    return (
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-card-lg p-8 space-y-6">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Join Tweenz AI</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Currently free during launch — no signup required</p>
        </div>
        ...
        <div className="rounded-xl bg-brand-50 dark:bg-brand-900/20 border border-brand-200 dark:border-brand-800 p-4">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-sm font-semibold text-brand-800 dark:text-brand-300">Instant access — no credit card</span>
          </div>
          <p className="text-xs text-brand-700 dark:text-brand-400 leading-relaxed">
            All features are unlocked during the launch phase. Click below to start
            immediately with a full pre-loaded MBA student account.
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            ...
            Start for Free — Enter Dashboard
          </button>
        ...
        <div className="grid grid-cols-2 gap-2.5 text-xs text-slate-500">
          {["5 MBA Courses loaded", "AI Tutor (RAG-powered)", "Lecture Transcription", "Group Workspace",
            "Achievements & badges", "Bilingual EN / AR", "Messages + AI replies", "Free — all features"].map(feat => (
        ...
        <p className="text-center text-xs text-slate-400">
          ...
          <Link href="/login" className="text-brand-600 hover:underline dark:text-brand-400">Log in</Link>
        </p>
      </div>
    );
  }

  // Non-demo signup form
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-card-lg p-8">
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl text-sm text-red-700 dark:text-red-400">{error}</div>
      )}

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t("auth.signup.name")}</label>
          <input name="name" type="text" required autoComplete="name" placeholder="Your full name" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t("auth.signup.email")}</label>
          <input name="email" type="email" required autoComplete="email" placeholder="your@email.com" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t("auth.signup.password")}</label>
          <div className="relative">
            ...
            <button type="button" onClick={() => setShowPw(!showPw)} className="absolute end-3 top-1/2 -translate-y-1/2 text-slate-400">
              {showPw ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t("auth.signup.confirm_password")}</label>
        ...
        <Button type="submit" fullWidth loading={loading}>{loading ? t("auth.signup.loading") : t("auth.signup.submit")}</Button>
      </form>
      ...
      <p className="mt-6 text-center text-sm text-slate-500">
        ...
        <Link href="/login" className="text-brand-600 hover:text-brand-700 font-medium dark:text-brand-400">{t("auth.signup.login_link")}</Link>
      </p>
    </div>
  );
}
