...
class Review(private val store: Store) {
    ...
    fun build(): ReviewReport {
        // بنحسب "جولات" حقيقية بس ونتجاهل "جديد" (regen).
        val decisions = store.feedback().filter { it.choice != "regen" }
        ...
        return ReviewReport(acceptRate, accepted, total, topThemes, store.styleExamples().size, worst)
    }
}
