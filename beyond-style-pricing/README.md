...
| Pricing Brain chat assistant | `/brain` |
...
4. Import this repo into Vercel with **Root Directory = `beyond-style-pricing`**.
5. Environment variables: `DATABASE_URL`, `AUTH_SECRET`, and optionally
   `AI_BASE_URL`, `AI_API_KEY`, `AI_TEXT_MODEL`, `AI_VISION_MODEL`.
6. Open the deployed URL on Android → browser menu → **Add to Home screen** (PWA).
