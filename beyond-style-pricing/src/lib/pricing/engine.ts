...
export function computePrice(
  ...
  if (!input.supplierCost && !materialCost) {
    ...
    warnings.push(
      warn(
        ...
        'critical',
        'No material cost and no supplier quote — cost base is unknown.',
        'لا توجد تكلفة خامة ولا عرض سعر من المورد — أساس التكلفة غير معروف.'
      )
    );
  }
  ...
  if (deliveryCost >= rules.deliveryRemote) {
    warnings.push(
      warn(
        ...
        `تكلفة التوصيل ${deliveryCost} درهم مرتفعة — يُنصح بإضافتها على العميل.`
      )
    );
  }
  ...
  const denom = 1 - f * (1 + m);
  if (denom <= 0) {
    throw new Error('Payment fee + commission too high for this margin — price is unsolvable.');
  }
    ...
    if (input.adminOverride) {
      warnings.push(
        warn('BELOW_COST_OVERRIDE', 'critical',
          ...
          'البيع أقل من التكلفة — تم بموافقة المدير فقط.'));
    } else {
      blocked = true;
      warnings.push(
        warn('BELOW_COST', 'critical',
          'Selling price is below cost. Approval blocked — admin override required.',
          'سعر البيع أقل من التكلفة. تم إيقاف الاعتماد — يلزم تجاوز من المدير.'));
    }
